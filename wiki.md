...
- `GET /api/chart` в каждой точке отдаёт `note` (заметка к замеру, обрезанная до
  200 в эндпоинте сохранения) и `available_months` для навигации по месяцам.
...
- **UX-мелочи из аудита:** busy-guard на `note-save` (кнопка `disabled` +
  ...
  (запрос `/api/chart` только при пустом кэше); при HTTP 401 (`Нет доступа`) —
  баннер `#auth-hint` «откройте приложение через Telegram» (`showAuthHint`,
