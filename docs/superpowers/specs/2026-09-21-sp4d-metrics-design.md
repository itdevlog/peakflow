...
- Веб по умолчанию слушает `127.0.0.1` (`WEBAPP_HOST`), поэтому `/metrics` не публичен;
  токен — дополнительная защита. Персональных данных в метриках нет (только числа).
...
| Риск | Решение |
|------|---------|
| Метрики утекают наружу | off by default, опциональный токен, веб на `127.0.0.1` |
