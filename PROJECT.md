...
| PDF-отчёт врачу | A4-отчёт (график, статистика, таблица) за неделю/месяц/квартал из бота и Mini App (SP4A) |
| Аналитика | Зоны, средний ПСВ по дням недели, тренд за 14 дней, корреляция заметок (SP5D, SP5E) |
| PWA | Установка на домашний экран и офлайн app-shell Mini App (SP5C) |
| Метрики | Расширенный `/healthz` (uptime, счётчики) и опциональный `GET /metrics` (SP4D) |
...
| `ZONE_YELLOW` | `60` | Порог жёлтой зоны (% от нормы); красная — ниже него |
| `WEEKLY_REPORT_DAY` | `6` | День недели отчёта (0=Пн, 6=Вс) |
| `WEEKLY_REPORT_HOUR` | `21` | Час отчёта |

> Часы напоминаний настраиваются в БД **на семью** (`get_reminder_hours`;
> дефолты — `database.REMINDER_HOURS_DEFAULT`, правит родитель через «⏰ Напоминания»).
...
| `chart` | `cb_chart(cb)` | График (по текущему месяцу, с навигацией ‹/›) |
...
`GET /api/analytics` (см. вкладку «Аналитика»).
...
`/api/settings`, `/api/export/*`, `/api/backup`, `/api/gamification`,
`/api/analytics`) scoped по
...
(заметка к замеру); `available_months` — поле верхнего уровня ответа для
навигации по месяцам. В табе
